...
class CheckpointSaver(HookBase):
    def __init__(self, save_freq=None, save_start_epoch=100000):
        self.save_freq = save_freq  # None or int, None indicate only save model last
        self.save_start_epoch = save_start_epoch

    def _should_save_epoch_checkpoint(self):
        if not self.save_freq:
            return False
        current_epoch = self.trainer.epoch + 1
        if current_epoch < self.save_start_epoch:
            return False
        return current_epoch % self.save_freq == 0
