...
class InsSegTester(TesterBase):
    ...
    def write_scannet_results(
        ...
        # Current SPFormer outputs original-point masks and original dataset class ids.
        ...
        if pred_masks.shape[1] != num_points:
            raise ValueError(
                f"Expected {num_points} points for {data_name}, "
                f"got masks with width {pred_masks.shape[1]}."
            )
        ...
        with open(result_path, "w") as result_file:
            for i, (score, mask, cls) in enumerate(
                ...
                cls = int(cls)
                if cls < 0 or cls >= len(class2id) or cls in self.segment_ignore_index:
                    raise ValueError(f"Invalid ScanNet class id {cls} for {data_name}.")
                label_id = int(class2id[cls])

                mask_path = os.path.join(mask_dir, f"{data_name}_{i:03d}.txt")
                relative_path = os.path.join("predicted_masks", f"{data_name}_{i:03d}.txt")
                np.not_equal(mask, 0).astype(np.uint8).tofile(
                    mask_path, sep="\n", format="%d"
                )
                with open(mask_path, "a") as mask_file:
                    mask_file.write("\n")
                result_file.write(f"{relative_path} {label_id} {float(score):.4f}\n")
    ...
    def write_scannetpp_results(
        ...
        for i, (score, mask, cls) in enumerate(
            ...
            result_file.write(f"{relative_path} {cls} {score:.4f}\n")
        result_file.close()
