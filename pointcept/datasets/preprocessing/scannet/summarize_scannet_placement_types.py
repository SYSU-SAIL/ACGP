"""Infer ScanNet 20-class placement types from geometry and support relations.
...
from meta_data.scannet200_constants import CLASS_LABELS_20, CLASS_LABELS_200
...
def parse_args():
    ...
    parser.add_argument("--output", default="outputs/scannet_placement_types/scannet_placement_types.json")
...
def instance_class_id(segment, mask):
    labels = segment[mask]
    labels = labels[(labels >= 0) & (labels < len(CLASS_LABELS_20))]
    if len(labels) == 0:
        return None
    counts = np.bincount(labels, minlength=len(CLASS_LABELS_20))
...
def update_scene_stats(scene_dir, stats, args):
    ...
    segment_path = scene_dir / "segment.npy"
    if not segment_path.exists():
        segment_path = scene_dir / "segment20.npy"
    segment = np.load(segment_path).reshape(-1).astype(np.int32)
    segment200_path = scene_dir / "segment200.npy"
    segment200 = (
        np.load(segment200_path).reshape(-1).astype(np.int32)
        if segment200_path.exists()
        else None
    )
    ...
    if segment200 is not None and len(segment200) != len(coord):
        raise ValueError(f"Mismatched segment200 array in {scene_dir}")
    if not np.all(np.isfinite(coord)):
        raise ValueError(f"Non-finite coordinates in {scene_dir}")

    wall_id = CLASS_LABELS_20.index("wall")
    floor_id = CLASS_LABELS_20.index("floor")
    ceiling_id = CLASS_LABELS_200.index("ceiling") if segment200 is not None else None
    wall_tree = make_tree(coord[segment == wall_id])
    floor_tree = make_tree(coord[segment == floor_id])
    ceiling_tree = make_tree(coord[segment200 == ceiling_id]) if ceiling_id is not None else None
...
def write_python(path, placement_types):
    with path.open("w") as file:
        file.write('"""Generated by summarize_scannet_placement_types.py."""\n\n')
        file.write("SCANNET_PLACEMENT_TYPES = {\n")
        for name in CLASS_LABELS_20:
