...
@MODELS.register_module("SPFormerMOEDecoder")
class SPFormerMOEDecoder(SPFormerDecoder):
    def __init__(
        self,
        num_layer=6,
        num_query=100,
        ...
        use_param_query=True,
        sp_moe_mlp_ratio=8,
        sp_moe_num_experts=8,
        sp_moe_num_experts_per_tok=2,
        sp_moe_pretraining_tp=1,
        sp_moe_mlp="FFN",
        decoder_moe_mlp_ratio=4,
        decoder_moe_num_experts=4,
        decoder_moe_num_experts_per_tok=2,
        decoder_moe_pretraining_tp=1,
        decoder_moe_mlp="FFN",
        use_decoder_moe=True,
        balance_loss=True,
    ):
        super().__init__(
            num_layer=num_layer,
            ...
            iter_pred=iter_pred,
            attn_mask=attn_mask,
            use_query_pos=use_query_pos,
            use_score=use_score,
            use_param_query=use_param_query,
        )
        self.use_decoder_moe = use_decoder_moe

        if self.use_decoder_moe:
            self.decoder_moe_layers = nn.ModuleList(
                [
                    SparseMoeBlock(
                        embed_dim=d_model,
                        mlp_ratio=decoder_moe_mlp_ratio,
                        num_experts=decoder_moe_num_experts,
                        num_experts_per_tok=decoder_moe_num_experts_per_tok,
                        pretraining_tp=decoder_moe_pretraining_tp,
    ...
    def forward(self, input_dict):
        ...
        for i in range(self.num_layer):
            ...
            if self.use_decoder_moe:
                query = self.decoder_moe_layers[i](query, SEP=False)
            pred_label, pred_score, pred_mask, attn_mask = self._forward_head(
                query, mask_feats
            )
            pred_labels.append(pred_label)
